...
def adjust_learning_rate(accelerator, optimizer, scheduler, epoch, args, printout=True):
    # Support both scheduler-driven updates and the original manual decay rules.
    lr_adjust = None
    if args.lradj == 'COS':
        scheduler.step()
    ...
    if lr_adjust and epoch in lr_adjust.keys():
        lr = lr_adjust[epoch]
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
    if printout:
        if accelerator is not None:
            accelerator.print(f'Updating learning rate to {lr:.8f}')
        else:
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path):
        # Track the best validation score and stop once patience is exhausted.
        score = -val_loss
        if self.best_score is None:
            ...
            if self.accelerator is None:
                ...
                self.accelerator.print(f'Early stopping counter: {self.counter}/{self.patience}')
...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    model.train()
    return total_loss, total_mae_loss,total_rmse,total_mape
